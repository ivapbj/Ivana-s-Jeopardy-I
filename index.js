...
let finalCategory = placeholderQuestions.filter((question) => {
  if (question.category === "Final") return true;
});

let natureCategory = placeholderQuestions.filter((question) => {
  if (question.category === "Nature") return true;
});

let animalCategory = placeholderQuestions.filter((question) => {
  if (question.category === "Animal") return true;
});
let computersCategory = placeholderQuestions.filter((question) => {
  if (question.category === "Computers") return true;
});
let mythologyCategory = placeholderQuestions.filter((question) => {
  if (question.category === "Mythology") return true;
});
console.log(mythologyCategory);
let historyCategory = placeholderQuestions.filter((question) => {
  if (question.category === "History") return true;
});
let generalCategory = placeholderQuestions.filter((question) => {
  if (question.category === "General") return true;
});
...
document.addEventListener("DomContentLoaded", function () {
  alert(`It's Player 1's turn to choose.`);
});

function disableGuessButtons() {
  // create button for passbuton and guessButton

  guessButton.diabled = true;
  passButton.diabled = true;
  nextRoundButton.diabled = true;
}
...
//   .addEventListener("change", enableButtons);
